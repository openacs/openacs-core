...
<else>
  ...
  <p> This is the <if @install:rowcount;literal@ eq 1>package</if><else>list of packages</else> we are going to install. </p>
...
<if @continue_url@ not nil>
  <p>
    <a href="@continue_url@" class="button">Install above <if @install:rowcount;literal@ eq 1>package</if><else>packages</else></a>
  </p>
</if>
<if @problems_p;literal@ true>
