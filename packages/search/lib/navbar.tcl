ad_include_contract {
    ...
    A navbar chunk for search.

} {
    {q ""}
    {paginator_class:token "list-paginator-bottom"}
    {count:integer}
    {low:integer}
    {high:integer}
...
set to_result_page      [expr {($last_result_page < $pages_per_group + $from_result_page - 1)
                               ? $last_result_page
                               : $pages_per_group + $from_result_page - 1} ]
