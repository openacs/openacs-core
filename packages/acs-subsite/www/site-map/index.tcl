...
db_foreach nodes_select {} {
	...
	if {$new_application == $node_id} {
	    ...
	    set action_form_part "[export_vars expand:multiple root_id node_id new_package_id] [apm_application_new_checkbox]"
	...
	set action_form_part "[export_vars expand:multiple root_id node_id rename_package_id]"
	...
	set action_form_part "[export_vars expand:multiple parent_id node_type root_id]"
    }
...
#set new_app_form_part_1 "<p align=\"top\"><form name=new_application action=package-new><input type=hidden name=node_id value=$node(node_id)><input type=hidden name=root_id value=$node(node_id)><input type=hidden name=new_node_p value=t>[export_vars expand:multiple]<input name=node_name type=text size=8></p>"
