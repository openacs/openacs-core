<master>
<property name="context">@context@</property>
<property name="title">Add Constraint on @segment_name@</property>

Now that you have created a relational segment, you can create
relational constraints on this segment. Relational constraints allow
you to apply rules on inter-party relationships.

<p>

Would you like to create a constraint now?

<p>

<center>
<include src="../confirm-delete-form" action="constraints-redirect" export_vars="@export_vars@" no_button=" No " yes_button=" Yes ">
</center>
