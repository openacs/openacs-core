...
set parent_link [export_vars {expand:multiple {root_id $parent_id}}]
...
db_foreach path_select {} {
    if {$node_id != $root_id && $admin_p == "t"} {
        append head [subst {<a href="[ns_quotehtml [export_vars {expand:multiple {root_id $node_id}}]]">}]
    }
    if {$name eq ""} {
    ...
    if {$directory_p == "t"} {
        ...
        if {$object_id eq ""} {
            ...
            if {[ad_conn node_id] != $node_id} {
                set unmount_url [export_vars {expand:multiple root_id node_id}]
            }
