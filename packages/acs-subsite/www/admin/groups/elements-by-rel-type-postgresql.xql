...
<fullquery name="relations_query">      
    ...
    select
       g.group_id, g.rel_type, g.rel_type_valid_p, gr.group_rel_id,
       ...
       rels.num_rels    
    from (
       select v.group_id, v.rel_type, 
              case when v.group_id = null then 0 else 1 end as rel_type_valid_p
       from rc_valid_rel_types v where v.group_id = :group_id
    ) g
...
<fullquery name="relations_query_OLD">      
      <querytext>
