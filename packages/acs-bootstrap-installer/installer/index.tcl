...
if { [ns_queryexists done_p] } {
    ...
    # browser's Reload button to get the main ACS login page once (s)he's
    # restarted the OpenNSD.
    
  if { [ad_verify_install] } {
    install_return 200 "Complete" "

You have successfully installed the ArsDigita Community System!

<p> Your server process has been terminated.  Unless you have configured your
server to restart automatically, as outlined in the 
<a href=\"http://acs40.arsdigita.com/doc/install-guide/acs.html#startup\">
ACS Installation Guide</a>, you will need to start your server again.
When the server restarts, the ACS will be fully functional and you can reload 
this page to access the running server.
"
    exit
  } else {
    install_return 200 "Error" "
The installation program has encounted an error.  Please drop your ACS tablespace
and the ACS username, recreate them, and try again.  You can log this as a bug
using the <a href=\"http://www.arsdigita.com/sdm/\">SDM</a>. 
"
    return
  }
  return
...
Thank you for installing the ArsDigita Community System,
a suite of fully-integrated enterprise-class solutions
for collaborative commerce.
This is the ACS Installer which performs all the steps necessary
to get the ArsDigita Community System running on your server.<p>
...
if { [catch { ns_sha1 quixotusishardcore }] } {
    ...
    required in ACS 4.0 so that passwords can be securely stored in
...
if { [string range [info tclversion] 0 0] < 8 } {
    ...
    for the ACS to work.  Probably your <code>nsd</code> executable is linked to <code>nsd76</code>.  Please
...
is required to support the ACS Security system.  Please make sure that <code>ns_cache</code>
...
After adding <code>ns_cache</code>, please restart your server.
</strong></p>"
    set error_p 1
} 
...
the ACS Templating System.  Without this templating system, none of the ACS pages installed by default
...
After adding support for the fancy ADP parser, please restart your server.
</strong></p>"
    set error_p 1
}   
...
if {$error_p} {
    append body "<p>
<strong>A number of misconfigurations were discovered that need to be corrected.
Please fix all of them, restart the server, and try running the installer again.
...
if { ![db_table_exists apm_packages] } {
    ...
    append body "<p>The next step is to install the ACS kernel data model. Click the <i>Next</i>
    button to proceed.
	...
	append body "<p>It appears that the ACS data model is only partially installed.
	Please drop your tablespace and start from scratch."
    } else {
	append body "<p>The ACS data model is already installed. Click <i>Next</i> 
