...
<li class="listitem"><p>
<span class="emphasis"><em>Server</em></span>-global: As you'd
...
server-global variables, use AOLserver 3's <a class="ulink" href="http://www.aolserver.com/docs/nsv.adp" target="_top">
<code class="computeroutput">nsv</code> API</a> (which supersedes <code class="computeroutput">ns_share</code> from the pre-3.0 API).</p></li><li class="listitem"><p>
...
(i.e., Tcl interpreter) between HTTP requests. If it didn't, that
would affect (and complicate) our use of script-global variables
dramatically, which would then be better described as <span class="emphasis"><em>thread</em></span>-global variables. Given
...
completely abort execution of the caller thread. <code class="computeroutput">return</code> doesn't work, because the procedure
may be nested several levels deep. We therefore use <a class="ulink" href="/api-doc/proc-view?proc=ad%5fscript%5fabort" target="_top"><code class="computeroutput">ad_script_abort</code></a> to
abort the remainder of the thread. Note that using <code class="computeroutput">return</code> instead of <code class="computeroutput">ad_script_abort</code> may raise some security
...
use of magic numbers, e.g.:</p><pre class="programlisting">
...
you'd use pass-by-reference.</p><div class="blockquote"><blockquote class="blockquote"><p><span class="emphasis"><em>The transformation of the array into
a list and back to an array takes, in our test environment,
approximately 10 microseconds per entry of 100 character's length.
Thus you can process about 100 entries per milisecond. The time
depends almost completely on the number of entries, and almost not
at all on the size of the entries.</em></span></p></blockquote></div><p>You implement pass-by-reference in Tcl by taking the name of an
...
Pass-By-Reference</p><p>An array is a type of <span class="emphasis"><em>set</em></span>, which means you can't have multiple
entries with the same key. Data structures that can have multiple
entries for the same key are known as a <span class="emphasis"><em>multiset</em></span> or <span class="emphasis"><em>bag</em></span>.</p><p>If your data can have multiple entries with the same key, you
...
do a case-insensitive lookup on an <code class="computeroutput">ns_set</code>, something you can't easily do on an
array. This is especially useful for things like HTTP headers,
which happen to have these exact properties.</p><p>You always use pass-by-reference with <code class="computeroutput">ns_set</code>s, since they don't have any built-in
way of generating and reconstructing themselves from a string
representation. Instead, you pass the handle to the set.</p><pre class="programlisting">
...
</pre><p>We don't recommend <code class="computeroutput">ns_set</code> as
...
have built-in syntactic support for them.</p><p>Consider for example a loop over the entries in a <code class="computeroutput">ns_set</code> as compared to an array:</p><pre class="programlisting">
...
number of entries is large.</p><div class="cvstag">($Id: programming-with-aolserver.xml,v 1.7
