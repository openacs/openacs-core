...
except for <kbd>src</kbd> are assumed to be arguments and are set as
...
<kbd>include</kbd> become available to the .tcl and .adp files being
include'd, but it <strong>does not</strong> make them settable through
<kbd>ad_page_contract</kbd>.
...
.tcl component does not call <kbd>ad_page_contract</kbd>.</p><p>If you'd like to include a full page (that is, one which calls
<kbd>ad_page_contract</kbd>) then instead of passing a parameter
through <kbd>&lt;include&gt;</kbd>, you could use <kbd><a href="http://openacs.org/api-doc/proc-view?proc=rp%5fform%5fput">rp_form_put</a></kbd>
...
</li><li>If the <kbd>src</kbd> attribute begins with a slash, the path is
