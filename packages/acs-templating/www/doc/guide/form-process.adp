...
<kbd>form create</kbd> command) and populates it with elements.</li><li>The developer may use the <kbd>form is_request</kbd> command to
encapsulate any special initialization (for example, setting a
primary key value for an <strong>Add</strong> form, or retrieving current
values for an <strong>Edit</strong> form).</li><li>The <kbd>formtemplate</kbd> tag is used to enclose the form
template. This tag is responsible for generating the appropriate
HTML <kbd>FORM</kbd> tag in the final output. The
<kbd>formtemplate</kbd> tag also embeds a special hidden variable in
the form for the purpose of identifying incoming submissions.</li><li>By default, the <kbd>formtemplate tag</kbd> sets the
