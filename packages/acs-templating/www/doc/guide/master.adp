...
<property name="context">{/doc/acs-templating {Templating}} {Templating System User Guide: Using Master Templates}</property>
<property name="doc(title)">Templating System User Guide: Using Master Templates</property>
<master>

<body>
<h2>Using Master Templates</h2><a href="..">Templating System</a> : <a href="../developer-guide.html">Developer Guide</a> : User Guide
...
implement a master template.</p><h3>Design a Content Frame</h3><p>Most web pages are laid out with a central <em>content area</em>
where the actual unique content of the page is displayed.
Surrounding the content area is a <em>frame</em> with common
elements that are consistent from page to page:</p><table cellspacing="0" cellpadding="10" border="1">
<tr>
...
</table><p>Most sites use an HTML table to delineate the content area
...
only have a header above and a footer below the content area.</p><p>The master template is typically highly dynamic. Menus, context
bars and other navigational controls must change depending on the
section of the site the user is browsing. A "Related Links" box
would have to reflect the specific contents of the page. The master
...
content area from the header and footer.</p><h3>Write the Page Template(s)</h3><p>A page template must include a <tt>master</tt> tag to specify
that its output should be enclosed in a master template:</p><blockquote><pre>
...
&lt;p&gt;&lt;b&gt;\@byline\@&lt;/b&gt;: \@text&lt;/p&gt;
...
</pre></blockquote><p>The <tt>master</tt> tag may be included anywhere in the body of
the page template, although usually the top of the file is the best
location for it.</p><h3>Adding Dynamic Elements to the Master Template</h3><p>The master template may be associated with its own Tcl script,
...
would include code like this:</p><blockquote><pre>
set user_name [your_procedure_to_get_the_current_user_name]
</pre></blockquote><p>The template would have a section like this:</p><blockquote><pre>
&lt;if \@user_name\@ nil&gt;
...
&lt;/else&gt;
</pre></blockquote><h3>Passing Property Values from the Page Template to Master
Template</h3><p>As mentioned above, in many cases the dynamic elements of the
...
master template:</p><blockquote><pre>
&lt;master src="/templates/master"&gt;
&lt;property name="title"&gt;\@title\@&lt;/property&gt;

&lt;!--Begin layout of page content--&gt;
...
</pre></blockquote><p>In this case, the <tt>property</tt> tag establishes
...
template may use to query for additional information.</p><hr><a href="mailto:templating\@arsdigita.com">templating\@arsdigita.com</a>
</body>
