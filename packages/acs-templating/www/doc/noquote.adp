...
appearance of the page. "Change the color of the table? How do I do
that when I cannot even find the body tag?" At this point some
suggest to embed Tcl code in the document rather than the other way
around, like PHP does. But it doesn't solve the problem, because
the code is still tightly coupled with the markup, requiring
programmer-level understanding for every change. The only workable
solution is to try to uncouple the presentation from the design as
much as possible.</p><p>ACS 4.0 addressed the problem by introducing a custom-written
...
text "&lt;i&gt;" is to transform it to "&amp;lt;i&amp;gt;". When a
browser renders the transformed text, entities &amp;lt; and
&amp;gt; are converted back to &lt; and &gt;, which makes the
rendered version of the transformation equal to the original.</p><p>The easiest way to guarantee correct transformation in all cases
is to "escape" ("quote") all the characters that HTML considers
special. In the minimalistic case, it is enough to transform &amp;,
&lt;, and &gt; into their quoted equivalents, &amp;amp;, &amp;lt;,
and &amp;gt; respectively. For additional usefulness in quoted
fields, it's a good idea to also quote double and single quotes
into &amp;quot; and &amp;#39; respectively.</p><p>All of this assumes that the text to be quoted is not meant to
be rendered as HTML in the first place. So if your text contains
"&lt;i&gt;word&lt;/i&gt;", and you expect the word to show up in
italic, you should not quote that entire string. However, if word
...
the database or from the user. This would "usually" work, but in
some cases it would lead to broken pages and even pose a security
problem. For instance, one could imagine a mathematicians' forum
being named "0 &lt; 1", or an HTML designers' forum being named
"The Woes of &lt;h1&gt;".</p><p>In some cases the published variable must not be quoted.
...
But we're not done yet: we need to write the Tcl code that grabs
forum names from the database. The db_multirow proc is designed
exactly for this; it retrieves rows from the database and assigns
variables from each row to template variables in each pass of a
multiple of our choice.</p><pre class="programlisting">
...
code that db_multirow was designed to avoid.</p><p>The alternative approach means less typing, but it's even uglier
in its own subtle way. The trick is to remember that our templating
...
"gotchas" that one can expect follows in a <a href="">separate
document</a> .</p><p>The discussion about speed, i.e. benchmarking results before and
