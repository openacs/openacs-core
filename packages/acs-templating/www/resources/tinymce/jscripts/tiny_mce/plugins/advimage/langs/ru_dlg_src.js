tinyMCE.addI18n("ru.advimage_dlg", {
    image_list: "Список картинок",
    align_right: "По правому краю",
    align_left: "По левому краю",
    align_textbottom: "По нижнему краю текста",
    align_texttop: "По верхнему краю текста",
    align_bottom: "По нижнему краю",
    align_middle: "По центру",
    align_top: "По верхнему краю",
    align_baseline: "По базовой линии",
    align: "Выравнивание",
    hspace: "Гориз. отступ",
    vspace: "Верт. отступ",
    dimensions: "Размер",
    border: "Граница",
    list: "Список",
    alt: "Описание",
    src: "Адрес",
    dialog_title: "Параметры изображения",
    missing_alt: "Продолжить без описания? Без этого изображение будет недоступно некоторым пользователям.",
    example_img: "Пример изображения",
    misc: "Дополнительные параметры",
    mouseout: "В покое",
    mouseover: "При наведении",
    alt_image: "Альтернативное изображение",
    swap_image: "Динамическое изображение",
    map: "Карта ссылок",
    id: "Имя элемента",
    rtl: "Справа налево",
    ltr: "Слева направо",
    classes: "Классы",
    style: "Стиль",
    long_desc: "Ссылка на полное описание",
    langcode: "Код языка",
    langdir: "Направление текста",
    constrain_proportions: "Сохранить пропорции",
    preview: "Педварительный просмотр",
    title: "Заголовок",
    general: "Общие параметры",
    tab_advanced: "Дополнительно",
    tab_appearance: "Положение",
    tab_general: "Общее"
});