tinyMCE.addI18n("uk.advlink_dlg", {
    target_name: "Назва цілі",
    classes: "Класи",
    style: "Стиль",
    id: "Ідентифікатор",
    popup_position: "Позиція (X/Y)",
    langdir: "Напрям мови",
    popup_size: "Розмір",
    popup_dependent: "Залежний (лише для Mozilla/Firefox)",
    popup_resizable: "Дозволяти змінювати розмір",
    popup_location: "Показувати панель location",
    popup_menubar: "Показувати меню",
    popup_toolbar: "Показувати панелі інструментів",
    popup_statusbar: "Показувати статус",
    popup_scrollbars: "Показувати скролери",
    popup_return: "Вставити 'return false'",
    popup_name: "Назва вікна",
    popup_url: "Посилання на виринаюче вікно",
    popup: "Виринаюче вікно Javascript",
    target_blank: "Відкрити в новому вікні",
    target_top: "Відкрити у верхньому фреймі (замінити всі фрейми)",
    target_parent: "Відкрити в батьківському вікні / фреймі",
    target_same: "Відкрити в цьому ж вікні / фреймі",
    anchor_names: "Якорі",
    popup_opts: "Властивості",
    advanced_props: "Розширені властивості",
    event_props: "Події",
    popup_props: "Властивості виринаючого вікна",
    general_props: "Загальні властивості",
    advanced_tab: "Додатково",
    events_tab: "Події",
    popup_tab: "Виринаюче вікно",
    general_tab: "Загальне",
    list: "Список посилань",
    is_external: "Введене посилання схоже на зовнішнє посилання, ви бажаєте додати потрібний префікс http://?",
    is_email: "Введене посилання схоже на адресу електронної пошти, ви бажаєте додати потрібний префікс mailto:?",
    titlefield: "Заголовок",
    target: "Відкрити в...",
    url: "Адреса",
    title: "Вставити/змінити посилання",
    link_list: "Список посилань",
    rtl: "Справа ліворуч",
    ltr: "Зліва праворуч",
    accesskey: "Клавіша доступу",
    tabindex: "Табіндекс",
    rev: "Зв'язок між ціллю та сторінкою",
    rel: "Зв'язок між сторінкою та ціллю",
    mime: "MIME тип цілі",
    encoding: "Кодування цілі",
    langcode: "Код мови",
    target_langcode: "Мова цілі"
});