...
<fullquery name="template::data::transform::party_search.search_persons">      
      <querytext>
        ...
        where lower(pe.first_names || ' ' || pe.last_name) like '%' || :search_string || '%'
        or lower(pa.email) like '%' || :search_string || '%'
...
<fullquery name="template::data::transform::party_search.search_groups_relsegs">
      <querytext>
        ...
        where lower(g1.group_name) like '%' || :search_string || '%'
        )
        union
        (
        ...
        where lower(g2.group_name || ' : ' || s2.segment_name) like '%' || :search_string || '%'
        )
        order by party_name
      </querytext>
</fullquery>
