...
ad_proc -public -callback search::summary -impl intermedia-driver {
    ...
    ns_log Notice "DIRK $query the query"
    # TODO implement intermedia::summary
    return [string range $text 0 2000]
}
...
namespace eval intermedia:: {}
namespace eval intermedia::install:: {}

ad_proc -public intermedia::install::package_install {
} {
    @author Dave Bauer (dave@thedesignexperience.org)
    @creation-date 2005-05-29

    Install APM callback, sets up service contract 
    implementations

} {
    intermedia::install::register_fts_impl
}

ad_proc -private intermedia::install::register_fts_impl {
} {
...
ad_proc -public intermedia::search {
    query
    offset
    limit
    user_id
    ...
    set results_ids [db_list search "select o.object_id from site_wide_index swi, acs_objects o where swi.object_id= o.object_id and contains (swi.indexed_content,:query)> 0 and and rownum > $offset and rownum <= $offset + $limit"]
    # TODO implement stopwords reporting for user query
    set count [db_string count "select count(swi.object_id) from site_wide_index swi, acs_objects o where o.object_id=swi.object_id and contains (swi.indexed_content,:query)> 0"]
    set stop_words ""
    ns_log notice "
-----------------------------------------------------------------------------
DAVEB intermedia::search
query = '{$query}'
return = '[list ids $results_ids stopwords $stop_words count $count]'
-----------------------------------------------------------------------------
"
