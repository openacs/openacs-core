...
</ul><h3>II. Introduction</h3><h3>III. Historical Considerations</h3><h3>V. Design Tradeoffs</h3><ul>
...
</ul><h3>VI. API</h3><h4>VI.A Locale API</h4><b>10.30</b> A Locale object represents a specific geographical,
...
be added to a locale, which we will omit in the Tcl API.</p><p>The <i>language</i> is a valid <b>ISO Language Code</b>. These
...
</p><p>The <i>country</i> is a valid <b>ISO Country Code</b>. These
...
</p><p>Examples are</p><blockquote>
<tt>en_US</tt> English US<br><tt>ja_JP</tt> Japanese<br><tt>fr_FR</tt> France French.</blockquote><p>The i18n module figures out the locale for a current request
makes it accessible via the <tt><b>ad_locale</b></tt> function:</p><pre>
...
"en_US"]</tt>, and an API for setting the locale with the
...
information accesible via the <tt>ad_conn</tt> function:
<pre><b>ad_conn locale</b></pre><h4>Character Sets and Encodings</h4>
We refer to <i>MIME character set names</i> which are the valid
values which can be passed in a MIME header, such as
<pre>
Content-Type: text/html; charset=<b>iso-8859-1</b>
</pre><p>You can obtain the preferred character set for a locale via the
<tt>ad_locale</tt> API shown below:</p><pre>
set locale "en_US"
...
name to a Tcl encoding name:</p><pre>
[<b>ns_encodingforcharset</b> "iso-8859-1"] =&gt; iso8859-1
</pre><h3>Templating</h3>
...
the same thing from within an ADP file.</p><h4>Caching multilingual ADP Templates</h4>
...
specific language such as <tt>foo.en.adp</tt>, but we need to
...
</h4><p>Let's say you have a template file "foo.adp" and it contains
calls to look up message strings using the TRN tag:</p><blockquote><pre>
&lt;master&gt;
...
If the user requests the page <tt>foo</tt>, and their
<b>ad_locale</b> is "en_US" then <i>effective locale</i> is
...
evaluated with the cached compiled page code.</p><p>The effective locale of a template page that has an explicit
...
effective locale of en_US.</p><h4>VI.B Naming of Template Files To Encode Language and Character
Set</h4><b>10.40</b> The templating system will use the Locale API to
...
<tt>filename.<i>locale_or_language</i>.adp</tt>.</p><p>Examples:</p><blockquote><pre>
foo.<i>en_US</i>.adp
foo.en.adp
...
</pre></blockquote><p>The user request has a <i>locale</i> which is of the form
...
"en_US".</p><p>The algorithm for finding the best matching template for a
request in a given locale is given below:</p><ol>
...
</ol><p>Once a template file is found we must decide what character set
it is authored in, so that we can correctly load it into Tcl (which
converts it to UTF8 internally).</p><p>It would be simplest to mandate that all templates are authored
...
ShiftJIS in Japan, or BIG5 in China.</p><p>So we make the convention that the template file is authored in
...
Eventually we can switch to using UTF8.</p><p>A character set corresponding to a locale can be found using the
...
before reading the template file from disk.</p><p>We read the template file using this encoding, and set the
...
it along to <b>ns_return</b>.</font></blockquote><p>The default character set for a template <tt>.adp</tt> file
should be the default system encoding.</p><h4>VI.C Loading Regular Tcl Script Files</h4><b>10.50</b> By default, tcl and template files in the system will
...
parameter value <tt>DefaultCharset</tt> if it is set.</p><p>Example AOLserver .ini configuration file to set default script
file and template file charset to ShiftJIS:</p><blockquote><pre>
...
</pre></blockquote><h3>VI.A Message Catalog API</h3>
...
example,</p><p>Message lookups are done using a combination of a key string and
...
language abbrev, such as <i>fr</i>, <i>en</i>, etc. The lookup
...
(or <tt>title</tt> for short), and the constant value
<tt>"Title"</tt> is supplied as documentation and as a default
...
catalog namespace using the <tt>lang_set_package</tt> command:</p><pre>
...
<tt>lang_set_package</tt> before doing a message lookup.
<h4>Message Catalog Definition Files</h4>
A message catalog is defined by placing a file in the
<tt>catalog</tt> subdirectory of a package. Each file defines a set
...
where <i>encoding</i> is the name of a MIME charset encoding
(<i>not</i> a Tcl charset name as was used in a previous version of
this command).
<pre>
/packages/bboard/catalog
                        /main.iso8859-1.cat
...
</pre><p>You can add more pseudo-levels of hierarchy in naming the
...
used inside of an ADP file. Here is the documention:</p><blockquote>Procedure that gets called when the &lt;trn&gt; tag is
...
</blockquote><h3>VII. Data Model Discussion</h3><h4>Internationalizing the Data Models</h4>
...
that you can specify in the queries.</p><h4>Drawbacks to Splitting Tables</h4><b>It is not totally transparent to developers</b><br>
...
table.</p><h3>VIII. User Interface</h3><h3>IX. Configuration/Parameters</h3><h3>X. Code Examples</h3><ul>
...
</ul><h3>XI. Future Improvements/Areas of Likely Change</h3><h3>XII. Authors</h3><ul>
<li>System creator</li><li>System owner: hqm\@arsdigita.com</li><li>Documentation author: hqm\@arsdigita.com</li>
</ul><h3>XII. Revision History</h3><p><i>The revision history table below is for this template -
modify it as needed for your actual design document.</i></p><table bgcolor="#EFEFEF" cellpadding="2" cellspacing="2" width="90%"><tbody>
<tr bgcolor="#E0E0E0">
...
</tbody></table><hr><a href="mailto:hqm\@arsdigita.com">hqm\@arsdigita.com</a><br>
</body>
