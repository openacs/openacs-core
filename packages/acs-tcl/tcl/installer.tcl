...
# when there's no Oracle driver available, or [ad_verify_install]
# returns false.
#
# If no Oracle driver is available, the acs-kernel libraries may not have
...
proc install_good_data_model_p {} {
	...
	if { [catch { db_string table_exists "select count(*) from $table_name" }] } {
	    return 0
	}
    }
    return 1
...
proc install_handler { conn arg why } {
    ...
    set path "[nsv_get acs_properties root_directory]/packages/acs-tcl/installer/$script.tcl"
    if { ![info exists path] } {
	install_return 404 "Not found" "
